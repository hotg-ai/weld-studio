 ...
 *   | "utf8"
  ...
  | "f64";
 */
export const arrowDataTypeToElementType = (
  _arrowDataType: string | Record<string, null>
): ElementType => {
  let data_type =
    typeof _arrowDataType === "string"
      ? _arrowDataType
      : Object.keys(_arrowDataType)[0];
  switch (data_type) {
    case "Utf8":
      return "utf8";
    case "Int8":
    ...
    default:
      throw new Error(
        `Unknown arrow data type: ${JSON.stringify(_arrowDataType)}`
      );
  }
};

export const arrowDataTypeToRunicElementType = (
  _arrowDataType: string | Record<string, null>
): RunicElementType => {
  let data_type =
    typeof _arrowDataType === "string"
