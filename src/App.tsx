...
function App() {
  ...
  const handleLoadArrowRowBatch = React.useCallback(
    (chunk: any[]) => {
      //setData[]
  ...
  React.useEffect(() => {
    ...
    listen("load_arrow_row_batch", ({ payload }: { payload: any[] }) =>
      handleLoadArrowRowBatch(payload)
    ).then((unsubscribe) => (handlerRegistered2 = unsubscribe));
  ...
  React.useEffect(() => {
    ...
    if (!handlerRegistered3) {
      listen("load_csv_complete", ({ payload }: { payload: any[] }) =>
        handleFetchTableUpdate(payload)
      );
